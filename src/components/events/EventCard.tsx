...
export function EventCard({ event }: { event: Event }) {
  ...
  return (
    <article className="rounded-2xl border border-border-warm bg-background-cream p-5 shadow-soft transition duration-200 hover:-translate-y-1 hover:shadow-warm">
      <div className="flex gap-4">
        <div className="flex h-20 w-16 shrink-0 flex-col items-center justify-center rounded-xl border border-border-warm bg-white text-brand-brown">
          <span className="text-2xl font-bold leading-none">{day}</span>
          <span className="mt-1 text-sm">{month}月</span>
        </div>
        <div>
          <p className="text-sm font-semibold text-brand-gold">{event.category}{event.isDemo ? " · 示意" : ""}</p>
          <h3 className="serif-title mt-1 text-xl font-bold text-brand-brown-dark">
